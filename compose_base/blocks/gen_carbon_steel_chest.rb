...
ctx = Compose::Context.new('compose_base/blocks/gen_carbon_steel_chest')
ctx.add_reference(nil, __FILE__)

Dir.glob(File.join(__dir__, "carbon_steel_chests", "*.json")) do |filename|
...
if ctx.modified
  ...
  target_dirname = File.join(__dir__, "../../compose_src/generated_blocks/carbon_steel_chest")
