...
ctx = Compose::Context.new('compose_base/blocks/gen_fluid_pipes')
ctx.add_reference(nil, __FILE__)

Dir.glob(File.join(__dir__, "fluid_pipes", "*.json")) do |filename|
...
if ctx.modified
  ...
  target_dirname = File.join(__dir__, "../../compose_src/generated_blocks/fluid_pipes")
